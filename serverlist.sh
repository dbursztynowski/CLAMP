...
while [  $COUNTER -lt  $MAX_COUNTER ]; do
    ...
    if openstack server list | grep -q "| test-resizevm-db | ACTIVE |" ; then
        ...
        echo "ACTIVE awaited after resize confirm..."
        sleep ${SLEEP_TIME}
    fi
done
...
if [ $ACTIVE -eq 1 ]; then
    echo "ACTIVE not entered in the loop. Current state is shown below."
fi

#ansible os_agent -m ansible.builtin.script "~/clamp/serverlist.sh"
